...
Після оплати WayForPay повертає покупця на `/thanks/<product>` (функція `api/thanks.mjs`): сторінка показує інвайт у канал курсу і рахує `Purchase`. Відкрита за прямим URL — доступ у канал захищає ручне схвалення заявок у Telegram.
